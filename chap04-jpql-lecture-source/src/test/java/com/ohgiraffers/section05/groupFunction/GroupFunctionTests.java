...
public class GroupFunctionTests {
    ...
    /*
    * 그룹함수
    * : JPQL에서 사용하는 그룹함수와 기존 SQL에서 사용하는 그룹함수는 차이가 거의 없다.
    * COUNT, SUM, AVG, MAX, MIN
    *
    * 주의사항
    * 1. 그룹함수의 반환 타입은 결과 값이 정수면 Long, 실수면 Double을 반환한다.
    * 2. 값이 없는 상태에서 COUNT를 제외한 그룹함수는 null이 되고 COUNT는 0이 된다.
    *   -> null로 받은 상태에서 long 또는 double 같은 기본자료형으로 받게 되면 언방식 과정에서 NPE이 발생한다.
    * 3. 그룹함수의 반환 자료형은 Long or Double 형이기 때문에
    *    Having 절에서 그룹함수 결과 값과 비교하기 위한 파라미터 타입은 Long or Double로 해야 한다.
    * => 받을 때 레퍼 타입을 사용해라!
    * */
